...
# Try to import cartopy for realistic basemap 
try:
    ...
    print("Warning: Cartopy not installed. Falling back to simple map.")
...
# read terrain data if available, otherwise use a Gaussian mountain as fallback
...
# geographic data calibration
# If terrain_bounds.txt exists, read it; otherwise use anchor-based manual calibration.
if os.path.exists("terrain_bounds.txt"):
    try:
        with open("terrain_bounds.txt", "r") as f:
            lines = f.readlines()
            # Expected format: "lon_min = 113.xxx".
...
if use_manual:
    # manual calibration using an anchor point and physical domain size
    # Anchor point: Tai Mo Shan
    anchor_lon = 114.12      # Tai Mo Shan longitude 
    anchor_lat = 22.41       # Tai Mo Shan latitude

    # Physical domain size of the model (meters)
    physical_width_km = 100.0   # LX = 100000 m
    physical_height_km = 100.0  # LY = 100000 m

    # Convert physical size to degrees at this latitude 
    # At 22.4°N: 1° lon ≈ 103 km, 1° lat ≈ 111 km
    ...
    delta_lon = 0.0    # positive shifts map east, negative west
    delta_lat = 0.0    # positive shifts map north, negative south

    # Compute the geographic extent
...
if CARTOPY_AVAILABLE:
    ...
    # show Tai Mo Shan anchor point (yellow star)
    ...
    # Logarithmic norm with a floor to avoid white from zeros
...
# Find the grid indices closest to anchor_lon, anchor_lat
...
# four corners of the terrain grid in geographic coordinates
if CARTOPY_AVAILABLE:
    # Mark the corners of the terrain matrix on the map
    # Blue point: matrix index [0,0] (top‑left)
    # Green point: matrix index [199,199] (bottom‑right)
    ...
    ax.plot(lon_max, lat_min, 'go', markersize=8, transform=ccrs.PlateCarree(),
            label='Matrix [199,199]')
    # Also mark the four corners of the extent box
...
# colorbar with INES level markers
...
# sliders for wind, source strength, height, and rain
...
def update_plot(frame):
    ...
    if not paused:
        ...
        if view_mode == 0:
            ...
            # Replace zeros with a very small value to avoid white in log scale
