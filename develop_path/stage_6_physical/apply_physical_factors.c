...
#define SCAVENGING_COEFF 1.0e-4   // s⁻¹ per mm/h
...
// Retrieve ground deposition map 
...
static void recompute_wind_fields(void) {
    ...
    for (int k = 0; k < NZ; k++) {
        ...
        if (z < 1000.0) {
            double theta = (z / 1000.0) * 0.5;      // 0 to 0.5 rad (~30°)
            ...
            for (int i = 0; i < NX; i++) {
                ...
                if (terrain_mask[idx]) {
                    ...
                    if (i > 0 && i < NX-1 && j > 0 && j < NY-1) {
                        ...
                        double h_xp = terrain_height(xp, y);
                        double h_xm = terrain_height(xm, y);
                        double h_yp = terrain_height(x, yp);
                        double h_ym = terrain_height(x, ym);
                        dh_dx = (h_xp - h_xm) / (2.0 * DX);
                        dh_dy = (h_yp - h_ym) / (2.0 * DY);
                    }

                    // Orographic lifting (vertical wind)
                    double terrain_z = terrain_height(x, y);
                    if (z < terrain_z + 500.0 && !terrain_mask[idx]) {
...
// Initialisation: allocate memory, precompute mask, then wind fields
EXPORT void init_simulation(void) {
    ...
    for (int k = 0; k < NZ; k++) {
        ...
        for (int j = 0; j < NY; j++) {
            ...
            for (int i = 0; i < NX; i++) {
                double x = i * DX;
                int idx = i + j*NX + k*plane;
                terrain_mask[idx] = (z < terrain_height(x, y)) ? 1 : 0;
            }
        }
    }
    ...
    // Stability check (using maximum possible wind speed and diffusion)
    double max_u = wind_profile(LZ, U_wind);
...
EXPORT void step_simulation(void) {
    ...
    // Precompute wet decay factor (constant for this time step)
    ...
    for (int k = 1; k < NZ-1; k++) {
        ...
        for (int j = 1; j < NY-1; j++) {
            for (int i = 1; i < NX-1; i++) {
                ...
                // LCG RNG (thread-safe, deterministic)
    ...
    C_new[0 + leak_y_idx*NX + leak_z_idx*plane] = source_strength;
    ...
    // Ground (z=0) – dry deposition (update ground deposition map)
    for (int i = 0; i < NX; i++) {
        for (int j = 0; j < NY; j++) {
